...
def create_grid(rows, cols, key_prefix):
    ...
    for i in range(rows):
        ...
        row = []
        for j in range(len(col_elements)):
            value = col_elements[j].text_input(f'{key_prefix} pos. {i+1},{j+1}', key=f'{key_prefix}_input_{i}_{j}')
            if value:
                try:
                    float(value)  # Verificar si el valor es un número
                except ValueError:
                    st.error(f'El valor en la posición {i+1},{j+1} no es un número válido.')
                    value = ''
            row.append(value)
        grid.append(row)
    return grid

# Función para verificar si todas las casillas están llenas y son números válidos
def all_fields_filled(grid):
    for row in grid:
        for cell in row:
            if not cell or len(cell) > 7:
